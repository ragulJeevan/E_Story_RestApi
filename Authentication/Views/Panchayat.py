...
from ..serializer import PanchayatSerializer,BulkPanchayatCreateSerializer
from django.http import Http404

modelName = "Panchayat"
notFound = "Panchayat not found"
ModelName = Panchayat
SerializerName = PanchayatSerializer
BulkSerialzier = BulkPanchayatCreateSerializer


class PanchayatList(APIView):
...
class BulkPanchayatCreateView(APIView):
    def post(self, request):
        serializer = BulkSerialzier(data=request.data)
        if serializer.is_valid():
            districts = serializer.save()
            district_response_data = SerializerName(districts, many=True).data
            return api_response(district_response_data, modelName, 'get')
        return api_error_response(serializer.errors)
    
class DeleteAllPanchayatView(APIView):
    def delete(self, request):
        ...
        return Response(
            {
                "message": "All Panchayats have been deleted successfully.",
